...
### Ключевые методы:
- `evaluate()` - публичный метод для вычисления выражений
- `_validate_parentheses()` - проверка корректности скобок
- `_tokenize()` - разбивка выражения на токены
- `_apply_operator()` - выполнение арифметических операций
...
## Обработка скобок в RPN

Калькулятор поддерживает использование круглых скобок `()` в RPN выражениях с строгой валидацией:

### Правила использования скобок:
- **Скобки должны быть сбалансированы** - каждая открывающая скобка должна иметь закрывающую
- **Запрещены пустые скобки** - между `(` и `)` должно быть выражение
- **Выражение в скобках должно быть завершено** - должно образовывать корректное RPN выражение

### Корректные примеры со скобками:
- (3 4 +) 2 * → результат: 14
- 3 (4 2 *) + → результат: 11
- (3 4 + 5 *) 2 / → результат: 17.5
...
#### 4. Некорректные примеры со скобками:
| Ошибка | Пример | Сообщение об ошибке | Причина |
|--------|--------|---------------------|---------|
| Несбалансированные скобки | `((3 4 +) 2 *` | `"Несбалансированные скобки"` | Незакрытые или лишние закрывающие скобки |
| Пустые скобки | `() 3 4 +` | `"Пустые скобки"` | Отсутствие выражения между скобками |
| Незавершенное выражение | `(4 +) 2 *` | `"Незавершенное выражение в скобках: 4 +"` | Неполное RPN выражение внутри скобок |
